...
import {InstanceProvider} from '../../cql/treeview/InstanceProvider';
import { beforeEach } from 'mocha';

suite('InstanceProvider Test Suite', () => {
	vscode.window.showInformationMessage('Start all tests.');

	let i: InstanceProvider;

	beforeEach('create the schema provider', () => {
		i = new InstanceProvider();
	});
	...
	test('getInstances error when given data without instances', () => {
		...
		assert.throws(() => { i.getInstances(data); }, 
			new Error("Instances not found."));
	});
	...
	test('getGenerators error when given data without generators', () => {
		...
		assert.throws(() => { i.getGenerators(data.instances.I.pres); }, 
			new Error("Generators not found."));
	}); 
	...
	test('getEquations error when given data without equations', () => {
		...
		assert.throws(() => { i.getEquations(data); }, 
