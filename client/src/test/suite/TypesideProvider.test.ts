...
import {TypeSideProvider} from '../../cql/treeview/TypeSideProvider';
import { beforeEach } from 'mocha';

suite('TypeSideProvider Test Suite', () => {
	vscode.window.showInformationMessage('Start all tests.');

	let t: TypeSideProvider;

	beforeEach('create the typeside provider', () => {
		t = new TypeSideProvider();
	});
	...
	test('getTypeSides error when given data without typesides', () => {
		...
		assert.throws(() => { t.getTypeSides(data); }, 
			new Error("Typesides not found."));
	});
	...
	test('getSymbols error when given data without symbols', () => {
		...
		assert.throws(() => { t.getSymbols(data); }, 
			new Error("Symbols not found."));
	});
	...
	test('getEquations error when given data without equations', () => {
		...
		assert.throws(() => { t.getEquations(data); }, 
			new Error("Equations not found."));
	});
